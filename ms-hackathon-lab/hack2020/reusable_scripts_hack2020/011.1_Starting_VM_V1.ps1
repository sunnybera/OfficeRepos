...
foreach ($VM in $VMs)
{
    ...
    Write-Output ($VM.Name + " being started")
	...
	if ($StartRtn.Status -ne 'VM Running')
	{
        ...
        Get-AzVM -ResourceGroupName $ResourceGroupName -Status | Select ResourceGroupName, Name, PowerState | Where {$_.PowerState –eq ‘VM running’} 
        
             
	}
	else
	{
		# The VM faied to start, so send notice
