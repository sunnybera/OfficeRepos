<#
*******************************************************************************************************************************************
Description      : This script will create B2B memberrs as per input csv.Invitation mail not sent to users.
*******************************************************************************************************************************************

Resource Type    : PaaS
Resource List    : AD Member, B2B Invitation Message to Member with invitation redeem url, 
Architecture     :
Best Practices   : Member can not be created with existing email in Directory, Invitation is set false as only members to be created in this script
...
foreach ($email in $invitations) {
    ...
    New-AzureADMSInvitation `
        ...
        -SendInvitationMessage $false ` #<==#EDITABLE -to be made true/false if mail trigger required/not required respectively
