...
Modified By   : Priti Bhunia
Modified Date : 28-March-2018
Description   : Added IIS extension installation and Boot diagnostic storage account details

Modified By   : Sunny Bera
Modified Date : 22-April-2019
Description   : Changed Naming convention for Caliber 2.0 Hackathon
                Updated StorageAccountType 

Modified By   : Suman Mukhopadhyay
Modified Date : 6-May-2019
...
Description   : Hard coding Admin Credential 
...
$usersList = Import-Csv "C:\11.1_cal2020_users" 
...
foreach($row in $usersList) {
    ...
    Write-Host "Creating Storage for Boot Diagnostics"
    $storagename = $rgname + 'calsta'
    New-AzureRmStorageAccount -StorageAccountName $storagename.ToLower() -ResourceGroupName $rgname -Location $Region -SkuName Standard_LRS
    Write-Host "Setting RmStorageAccount"
    $vm = Set-AzureRmVMBootDiagnostics -VM $VM -Enable -ResourceGroupName $rgname -StorageAccountName $storagename
